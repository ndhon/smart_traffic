...
void loop() {
  ...
  {
    red_time = Arduino.parseInt();
    if (red_time != 0 )
    {
      red_old = red_time;
      if (red_old == short_lane)
        cycle = red_old * 2;
      else if (red_old == long_lane)
        cycle = red_old * 2;
      else
        cycle = medium_cycle;
    }
    ...
    while (red_time == 0)
    {
      ...
      show_control(red_old, (cycle - (red_old + 3)), yellow);
      red_time = Arduino.parseInt();
      if (red_time != 0)
      {
        red_old = red_time;
...
void show_control(int red, int green, int yellow )
{
  while (red >= 0)
  {
